  ...
  expect(
    towns.every((town) => {
      const hasCorrectProperties =
        town.hasOwnProperty("coordinates") && town.hasOwnProperty("name");
      const onlyHaveTwoProperties = Object.keys(town).length === 2;
      const propertiesHasCorrectType =
        Array.isArray(town.coordinates) && typeof town.name === "string";
      const hasCoordinatesWithCorrectType = town.coordinates.every(
